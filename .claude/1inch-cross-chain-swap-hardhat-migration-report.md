...
2. **File upstream coverage bug** — 3 tests fail under `--coverage` because coverage instrumentation injects calls that overwrite the EVM returndata buffer inside library functions using inline assembly (`returndatasize` / `returndatacopy`). Minimal repro in `test/bugs/CoverageReturndataBug.t.sol`, full analysis in `.claude/bugs/coverage-returndata-bug.md`. Consider filing on [NomicFoundation/hardhat](https://github.com/NomicFoundation/hardhat/issues).
